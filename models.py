...
class TextCard_Detector(Yolo5):

    def __init__(self, weight_path):
        self.classes = text_classes = ['bottom_left', 'bottom_right', 'top_right', 'top_left', 'id_no', 'cid_no',\
    ...
    def post_process(self, input_image, outputs):
        ...
        indices = cv2.dnn.NMSBoxes(boxes, confidences, self.CONFIDENCE_THRESHOLD, self.NMS_THRESHOLD)
        for i in indices:            
            cls = self.classes[class_ids[i]]
            if cls[0] == 'c' and cls != 'class':  
                box = boxes[i]
                top_left = [box[0]-bias, box[1]-bias]
                bottom_right = [box[0]+box[2]+bias, box[1]+box[3]+bias]
                outputs.append([self.classes[class_ids[i]], [top_left, bottom_right]])
                cv2.rectangle(input_image, top_left, bottom_right, color=(0, 0, 255), thickness=2)
                #cv2.putText(input_image, self.classes[class_ids[i]], top_left, fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=1, color=(0,0,255), thickness=2)
        return input_image, outputs


class Text_Recognitor:

    config = Cfg.load_config_from_name('vgg_seq2seq')
    #config['weights'] = 'https://drive.google.com/uc?id=13327Y1tz1ohsm5YZMyXVMPIOjoOA0OaA'
    #config['weights'] = 'https://drive.google.com/file/d/1pXftFiTGzcXNqsy6jKxQF2WyiOoBmDKU'
    config['device'] = 'cuda'
